...
// ── Sub-command: serve ────────────────────────────────────────────────────────
// `murmur serve` runs the headless Node entrypoint without resolving Electron.
if (userArgs[0] === 'serve') {
  const serveEntry = path.join(pkgRoot, 'dist', 'main', 'serve.js');
  ...
  const child = spawn(process.execPath, [serveEntry, ...userArgs], {
    stdio: 'inherit',
    cwd: process.cwd(),
    shell: false,
  });
  ...
  child.on('error', (err) => {
    console.error('[murmur] failed to spawn serve:', err);
    process.exit(1);
  });
} else {
